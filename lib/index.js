...
const addHandler = (handler, name) => {
	if (
		...
		if (!REDUCERS[name]) {
			...
			if (ls && (!(ls.shouldLoad instanceof Function) || ls.shouldLoad())) {
				let localSavedData;
...
const dispatch = (action, data) => {
	...
	if (
		...
		let result = reducers[type](
			STORAGES[name],
			type != 'reset' ? data : getSavedState(name)
		);
		if (result instanceof Object) {
			if (type == 'reset' || type == 'init') {
				STORAGES[name] = result;
				return distributeChanges(name, type == 'init');
			}
...
const getSavedState = (name) => {
	...
	if (ls instanceof Object) {
		return STORE_KEEPER.get(ls.key);
	}
	return null;
}
...
const distributeChanges = (name, initial = false) => {
	...
	const ls = LOCAL_STORES[name];
	if (!initial && ls instanceof Object) {
		if (ls.shouldRemove instanceof Function && ls.shouldRemove(STORAGES[name])) {
			STORE_KEEPER.remove(ls.key);
		} else if (!(ls.shouldSave instanceof Function) || ls.shouldSave(STORAGES[name])) {
			let {timeout} = ls;
...
const subscribe = (updater, {has, handlers, flat, withPrefix}) => {
	if (typeof flat != 'boolean') {
		flat = DEFAULT_PARAMS.flat;
	}
	if (typeof withPrefix != 'boolean') {
		withPrefix = DEFAULT_PARAMS.withPrefix;
	}
	if (has === '*') {
...
const connect = (ComponentToConnect, connectProps) => {
	let ready = false;
	if (typeof connectProps == 'string' || connectProps instanceof Array) {
		connectProps = {
			has: connectProps
		};
	}
	...
	return class XStoreConnect extends reactComponent {
		...
		static displayName = 'XS_' + ComponentToConnect.name;
