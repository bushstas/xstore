...
export const dispatch = (name, action, payload) => {
...
function addReducer(reducer, name) {
	if (reducer instanceof Function && !REDUCERS[name]) {
	...
	if (use instanceof Array) {
		...
		use.forEach(name => {
			if (typeof name === 'string') {
				...
				SUBSCRIBERS[name].push(subscribers);					
			}
		});
	}
